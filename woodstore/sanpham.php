...
$sql = "SELECT SanPham.*, Loai.TenLoai 
        FROM SanPham 
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto bg-white p-6 rounded shadow">
        <h1 class="text-2xl font-bold text-black mb-6 text-center">Quản lý sản phẩm</h1>

        <div class="text-right mb-4">
            <a href="them_sanpham.php" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">+ Thêm sản phẩm</a>
        </div>

        <table class="min-w-full border-collapse border border-gray-300 ">
            <thead>
                <tr class="bg-amber-600 text-black">
                    <th class="py-2 px-4 border text-white">Mã</th>
                    <th class="py-2 px-4 border text-white">Tên sản phẩm</th>
                    <th class="py-2 px-4 border text-white">Loại</th>
                    <th class="py-2 px-4 border text-white">Kích thước</th>
                    <th class="py-2 px-4 border text-white">Chất liệu</th>
                    <th class="py-2 px-4 border text-white">Giá</th>
                    <th class="py-2 px-4 border text-white">Mô tả</th>
                    <th class="py-2 px-4 border text-white">Hành động</th>
                </tr>
            </thead>
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr class="border-t hover:bg-gray-50">
                        <td class="py-2 px-4 border"><?= $row['MaSP'] ?></td>
                        <td class="py-2 px-4 border"><?= htmlspecialchars($row['TenSP']) ?></td>
                        <td class="py-2 px-4 border"><?= htmlspecialchars($row['TenLoai']) ?></td>
                        <td class="py-2 px-4 border"><?= $row['Dai'] . ' x ' . $row['Rong'] . ' x ' . $row['Cao'] ?> cm</td>
                        <td class="py-2 px-4 border"><?= htmlspecialchars($row['ChatLieu']) ?></td>
                        <td class="py-2 px-4 border text-right"><?= number_format($row['Gia'], 0, ',', '.') ?>₫</td>
                        <td class="py-2 px-4 border"><?= htmlspecialchars($row['MoTa']) ?></td>
                        <td class="py-2 px-4 border text-center">
                            <a href="sua_sanpham.php?id=<?= $row['MaSP'] ?>" class="text-blue-600 hover:underline mr-2">Sửa</a>
                            <a href="xoa_sanpham.php?id=<?= $row['MaSP'] ?>" class="text-red-600 hover:underline" onclick="return confirm('Bạn có chắc muốn xóa sản phẩm này?')">Xóa</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
        ...
        <div class="text-center mt-6">
            <a href="user.php" class="text-amber-700 hover:underline">← Quay lại trang chính</a>
        </div>
    </div>
