...
// Truy vấn toàn bộ đơn hàng (có thể thêm phân trang, lọc trạng thái...)
$sql = "SELECT * FROM ChiTietDonHang ORDER BY Ngay DESC";
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto">
        <h1 class="text-3xl font-bold text-center text-black mb-8">Quản lý đơn hàng</h1>
        ...
        <table class="w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead>
                <tr class="bg-amber-600 text-white">
                    ...
                    <th class="py-3 px-4 text-left">Giao Dịch</th>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr class="border-b hover:bg-gray-100">
                        ...
                        <td class="border px-3 py-2"><?= htmlspecialchars($row['KH']) ?></td>
                        ...
                        <td class="border px-3 py-2"><?= htmlspecialchars($row['DiaChiGiao']) ?></td>

                        <td class="py-3 px-4">
                            <!-- Thêm nút sửa/xóa nếu muốn -->
                            <a href="admin_donhang_chitiet.php?id=<?= $row['MaDH'] ?>" class="text-blue-600 hover:underline mr-2">Chi tiết</a>
                            <a href="delete_user.php?id=<?= $row['MaDH'] ?>" class="text-red-600 hover:underline" onclick="return confirm('Bạn có chắc chắn muốn xóa đơn hàng này?')">Xóa</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
