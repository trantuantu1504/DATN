...
$sql = "
    SELECT
        ...
        KhachHang.MatKhau
    FROM TK
    JOIN KhachHang ON TK.MaTK = KhachHang.MaTK
";
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto">
        <h1 class="text-3xl font-bold text-center text-black mb-8">Quản lý tài khoản</h1>

        <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead>
                <tr class="bg-amber-600 text-white">
                    <th class="py-3 px-4 text-left">Mã TK</th>
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-100">
                    <td class="py-3 px-4"><?= htmlspecialchars($row['MaTK']) ?></td>
                    <td class="py-3 px-4"><?= htmlspecialchars($row['Email']) ?></td>
                    <td class="py-3 px-4"><?= htmlspecialchars($row['HoTen']) ?></td>
                    <td class="py-3 px-4"><?= htmlspecialchars($row['MatKhau']) ?></td>
                    <td class="py-3 px-4"><?= htmlspecialchars($row['DienThoai']) ?></td>
                    <td class="py-3 px-4"><?= htmlspecialchars($row['DiaChi']) ?></td>
                    <td class="py-3 px-4"><?= htmlspecialchars($row['Quyen']) ?></td>
                    <td class="py-3 px-4"><?= $row['TrangThai'] ? 'Hoạt động' : 'Khóa' ?></td>
                    <td class="py-3 px-4">
                        <!-- Thêm nút sửa/xóa nếu muốn -->
                        <a href="edit_user.php?id=<?= $row['MaTK'] ?>" class="text-blue-600 hover:underline mr-2">Sửa</a>
                        <a href="delete_user.php?id=<?= $row['MaTK'] ?>" class="text-red-600 hover:underline" onclick="return confirm('Bạn có chắc chắn muốn xóa tài khoản này?')">Xóa</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            </tbody>
